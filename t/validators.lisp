...
(define-test auto-validators-adds-not-blank-when-required ()
  (let* ((meta (shiso/models:make-slot-metadata
                :name 'title :col-type :text :blankp nil))
         (validators (shiso/validators:auto-validators-for-field meta)))
    (assert-true (member 'shiso/validators:not-blank validators)
                 "Required field should get not-blank validator")))

(define-test auto-validators-skips-not-blank-when-blankp ()
  (let* ((meta (shiso/models:make-slot-metadata
                :name 'bio :col-type :text :blankp t))
         (validators (shiso/validators:auto-validators-for-field meta)))
    (assert-false (member 'shiso/validators:not-blank validators)
                  "Blank-allowed field should not get not-blank")))

(define-test auto-validators-adds-max-length-for-varchar ()
  (let* ((meta (shiso/models:make-slot-metadata
                :name 'title :col-type '(:varchar 200)))
         (validators (shiso/validators:auto-validators-for-field meta)))
    (assert-true (find-if (lambda (v)
                            (and (listp v)
                                 (eq (car v) 'shiso/validators:max-length)
                                 (= (cadr v) 200)))
                          validators)
                 "varchar(200) should get max-length 200 validator")))

(define-test auto-validators-adds-one-of-for-choices ()
  (let* ((meta (shiso/models:make-slot-metadata
                :name 'status :col-type '(:varchar 20)
                :choices '((:draft . "Draft") (:published . "Published"))))
         (validators (shiso/validators:auto-validators-for-field meta)))
    (assert-true (find-if (lambda (v)
                            (and (listp v)
                                 (eq (car v) 'shiso/validators:one-of)))
                          validators)
                 "Choices field should get one-of validator")))
